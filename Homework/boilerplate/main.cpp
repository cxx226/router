...
extern RoutingTableEntry router_table[100]; // 路由�?
extern uint32_t router_table_len;  // 路由表长�?
extern uint32_t masks[33];  // len转换成mask的数�?
// 组播的ip地址
...
void set_rip_entry(RipEntry& t, RoutingTableEntry& s) {
  t.addr = s.addr;
  t.mask = ntohl(masks[s.len]); // 注意小端序转换成大端�?  
  t.nexthop = s.nexthop;
  t.metric = s.metric;
}
  ...
  while (1) {
    ...
    if (time > last_time + 5 * 1000) {
      ...
      for (uint32_t i = 0; i < N_IFACE_ON_BOARD; i++) {
        ...
        RipPacket rip;    // rip�?        
        rip.command = 2;  // response报文的command�?
        // 组装rip packet
        ...
        uint32_t length = 32 + cnt * 20;  // ip包总长�?        
    ...
    if (dst_is_me) {
      ...
      if (disassemble(packet, res, &rip)) {
        if (rip.command == 1) {
          ...
          // �?0s计时器中的response组装逻辑基本相同
          // 区别在于这次只给src_addr发，不需要每个端口都发�?          
          init_ip_udp_head();
          uint32_t cnt = 0;
          RipPacket resp;
          resp.command = 2;
          ...
          for (int i = 0; i < rip.numEntries; i++) {
            ...
            if (idx >= 0) {  // 若查找到则为表项序号，否则为-1
              RoutingTableEntry& rte = router_table[idx]; // 查找到的表项的引�?              
